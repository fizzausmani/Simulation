...
# pos0 = np.load('C:\\Users\\fusmani\\Box\\Research\\Python\\Full Results\\SP4\\10x10\\1\\FM MP T1000 N4 sd10 pos.npy')[:,:,0]
# p0 = np.load('C:\\Users\\fusmani\\Box\\Research\\Python\\Full Results\\SP4\\10x10\\1\\FM MP T1000 N4 sd10 p.npy')[:,:,0]
save_path = 'C:\\Users\\fusmani\\Box\\Research\\Python\\Full Results\\SP4\\10x10\\5\\'
...
sd = 0.1 # Saffman-Delbruck length: large for 2D, small for 3D
...
def dp_particle(ux_st, uy_st, pos, p):
    ux_interp = interp.RectBivariateSpline(x, y, ux_st, kx = 3, ky = 3)
    uy_interp = interp.RectBivariateSpline(x, y, uy_st, kx = 3, ky = 3)
    pdot = np.zeros([2,N])
...
df = pd.DataFrame([['T', T], ['dt', dt], ['Steps', steps], ['Box dimension, a', a], ['Grid spacing', dx], ['Meshgrid size, z', z],
                   ['Number of particles', N], ['System, free or confined', system], ['sd', sd], ['Confinement height, H', H],
